...
We have since confirmed with Gold Standard staff that only projects using Approach 1 require direct soil sampling to calculate the number of credits earned, with Approaches 2 and 3 relying on model and parameter-based calculations instead. Furthermore, we confirmed that, contrary to Mr. Hewlett’s assertion, Approaches 2 and 3 do _not_ require soil sampling to validate the accuracy of the model- or calculation-based methods they use. Thus, our original interpretation that the protocol does not require soil sampling either as a basis for crediting or for model validation is correct, and Mr. Hewlett’s public comments are not.
